...
export function RunOfShowReviewModal({
  onClose,
  onJumpToBeat,
}: {
  onClose: () => void;
  ...
  const run = async () => {
    setLoading(true);
    setError("");
    const result = await reviewRunOfShow(PROJECT_SLUG);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-lg font-semibold gold-text">✨ AI review — run of show</h3>
          <button onClick={onClose} className="btn px-3 py-1.5">
            ✕
          </button>
        </div>
        <p className="text-[12.5px] text-[var(--text-dim)] mb-4">
          Reads the full timeline for energy dips, missing leads/talent, and timing risks.
