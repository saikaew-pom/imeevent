...
import { listLibraryActs } from "@/lib/builder/companyLibrary";
...
// viewers (always role "viewer"), who should see only this project's own
// Show & Decor items, never the company's cross-project shared library
// (including its cost figures for acts belonging to other client projects).
export async function GET(req: NextRequest) {
  ...
  const items = await listLibraryActs(access.companyId);
